...
generate_spline_model_code <- function(weight_prior_sd) {
  ...
  "
  model_code <- glue::glue(model_code, weight_prior_sd = weight_prior_sd, .open = "{{", .close = "}}")
  model_code
}
```
...
fit_all_models <- function(models, basis_splines) {
  ...
  for (i in seq_along(basis_splines)) {
    ...
    for (j in seq_along(models)) {
      ...
      models_cb_fit[[glue::glue("spline:{spline_type}--weight_sd:{model_type}")]] <- models[[j]]$optimize(
