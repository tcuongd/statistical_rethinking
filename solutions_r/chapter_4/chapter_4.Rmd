...
```
The higher number of knots (`spline:higher`) doesn't actually seem to change the fit much for a given standard deviation prior on the weights. I'm guessing that 15 is a suitable number of knots for the range of years we're fitting, and a higher number of knots doesn't add much information.

Changing the standard deviation prior does have a big impact though. A narrow prior effectively penalizes the fitting process and ensures the fitted line isn't too "wiggly". This is because a narrow prior requires more data to move the mean of `w` away from zero.

We can see that the model with the wide standard deviation prior produces a more wiggly fit.
